...
> ⚠️ **PowerShell을 닫았다가 다시 열었을 때 (컨테이너 재접속)**
>
> `docker run ...`은 **이미지로 새 컨테이너를 만들면서 실행**하는 명령이므로, 다시 실행하면
> `ros_jazzy1` 이름이 이미 존재한다는 오류가 나며 실패합니다. 또한 `docker run -it`으로 붙어
> 있던 PowerShell을 닫으면 컨테이너도 함께 종료(Exited)되는 경우가 많습니다.
>
> 이때는 새로 만들지 말고 **기존 컨테이너를 재사용**합니다.
>
> ```bash
> docker ps -a                              # 컨테이너 상태 확인 (Exited 또는 Up)
> docker start ros_jazzy1                   # 종료(Exited)되어 있으면 재시작
> docker exec -it ros_jazzy1 bash           # 컨테이너 접속 (02_8과 동일한 명령)
> ```
>
> ※ `docker run`은 **최초 1회만** 실행하면 됩니다. 이후에는 위의 `start` + `exec`로 접속하며, 여러 창을 동시에 접속하려면 02_8의 방법을 사용합니다.
