...
* ROS에서 사용하는 로봇 모델링 방식을 URDF라고 하며, Unified Robot Description Format의 약자입니다.
* URDF는 로봇의 기하학적 형태, 질량 특성뿐 아니라 로봇을 구성하는 각 관절(Joint)의 특성까지 명확하게 표현할 수 있습니다.
* URDF 모델 파일은 XML 형식을 사용합니다.
* 로봇 모델링 과정은 모델링 언어를 사용하여 로봇의 각 구성 요소를 명확하게 정의한 다음, 정의된 아이디어에 따라 그것들을 결합하는 과정이라고 할 수 있습니다.

<img src="../img/image3.bmp" width="30%">

## 03_2 URDF 문법 살펴보기

### 링크(Link) 기술하기

* `<link>` 태그는 로봇의 강체(rigid body) 부분의 외형과 물리적 속성을 기술하는 데 사용됩니다. 
* 외형에는 크기, 색상, 형태가 포함되고, 물리적 속성에는 질량(mass), 관성 행렬(inertia matrix), 충돌(collision) 파라미터 등이 포함됩니다.
